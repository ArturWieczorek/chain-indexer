# Progress

A checklist of the course chapters. Each chapter is a single commit tagged
`chNN`. Tick a box when its tag exists and its tests are green.

- [x] ch00 - Orientation (scaffold, tooling, smoke test)
- [x] ch01 - The block and chain model
- [x] ch02 - The fork problem
- [x] ch03 - SQLite schema and store
- [x] ch04 - Indexing transactions
- [x] ch05 - Rollbacks and reorgs
- [x] ch06 - Shelley staking
- [x] ch07 - Conway governance
- [x] ch08 - A source and the Ogmios client
- [x] ch09 - The sync loop
- [x] ch10 - CBOR and real blocks
- [x] ch11 - Ouroboros wire I (mux and handshake)
- [x] ch12 - Ouroboros wire II (chain-sync)
- [x] ch13 - The query API
- [x] ch14 - The CLI
- [x] ch15 - The explorer
- [x] ch16 - Live view and analytics
- [x] ch17 - A real cluster and forced reorgs
- [x] ch18 - Design and tradeoffs
- [x] ch19 - Publish (v1.0.0)
- [x] ch20 - Local-state-query (v1.2.0)
- [x] ch21 - Epochs and the dashboard (v1.3.0)
- [x] ch22 - Pools (v1.4.0)
- [x] ch23 - Governance (v1.5.0)
- [x] ch24 - Live stake and saturation (v1.6.0)
- [x] ch25 - Bech32 addresses (v1.7.0)
- [x] ch26 - Accounts and rewards (v1.8.0)
- [x] ch27 - Account links in the UI (v1.9.0)
- [x] ch28 - Transaction detail (v1.10.0)
- [x] ch29 - Controlled stake (v1.11.0)
- [x] ch30 - Analytics (v1.12.0)
- [x] ch31 - Tokens (v1.13.0)
- [x] ch32 - Governance from CBOR (v1.14.0)
- [x] ch33 - Clickable governance and epoch blocks (v1.15.0)
- [x] ch34 - Certificates browser (v1.16.0)
- [x] ch35 - Transaction detail tabs (v1.17.0)
- [x] ch36 - Policy pages and readable asset names (v1.18.0)
- [x] ch37 - Protocol parameters and transaction governance links (v1.19.0)
- [x] ch38 - The constitutional committee (v1.20.0)
- [x] ch39 - Reward withdrawals (v1.21.0)
- [x] ch40 - Top addresses and staking accounts (v1.22.0)
- [x] ch41 - Transactions section (v1.23.0)
- [x] ch42 - Analytics time series (v1.24.0)
- [x] ch43 - The mempool (local-tx-monitor) (v1.25.0)
- [x] ch44 - Protocol updates (v1.26.0)
- [x] ch45 - Submitting transactions (local-tx-submission) (v1.27.0)
- [x] ch46 - CIP-25 asset metadata (v1.28.0)
- [x] ch47 - CIP-68 asset metadata (v1.29.0)
- [x] ch48 - Richer pool detail (v1.30.0)
- [x] ch49 - Mint transactions (v1.31.0)
- [x] ch50 - Full pool on-chain details (v1.32.0)
- [x] ch51 - Off-chain pool metadata (v1.33.0)
- [x] ch52 - Asset images (v1.34.0)
- [x] ch53 - Favicons, theme, and colour (v1.35.0)
- [x] ch54 - Richer recent blocks on the pool page (v1.36.0)
- [x] ch55 - Per-epoch stake history (v1.37.0)
- [x] ch56 - Governance tabs (v1.38.0)
- [x] ch57 - Header status and theme label (v1.39.0)
- [x] ch58 - A configuration file (v1.40.0)
- [x] ch59 - Pool performance: expected vs made, and saturation (v1.41.0)
- [x] ch60 - Live status colour (v1.42.0)
- [x] ch61 - Chart axes and gridlines (v1.43.0)
- [x] ch62 - A readable blocks-per-epoch chart (v1.44.0)
- [x] ch63 - A Postgres backend (v1.45.0)
- [x] ch64 - A kupo-style matches API (v1.46.0)
- [x] ch65 - A configurable bind address, run many networks (v1.47.0)
- [x] ch66 - Periodic terminal progress (v1.48.0)
- [x] ch67 - Datums by hash (v1.49.0)
- [x] ch68 - Richer events and an event filter (v1.50.0)
- [x] ch69 - A webhook sink (v1.51.0)
- [x] ch70 - A granular live feed (v1.52.0)
- [x] ch71 - Industry-style API docs (Swagger + API.md) (v1.53.0)
- [x] ch72 - Readable CIP-68 names and graceful images (v1.54.0)
- [x] ch73 - Reference scripts (v1.55.0)
- [x] ch74 - A Back link on every detail page (v1.56.0)
- [x] ch75 - Clickable datum and script hashes (v1.57.0)
- [x] ch76 - Log and file event sinks (v1.58.0)
- [x] ch77 - No-cache the explorer pages (v1.59.0)
- [x] ch78 - A chainidx events CLI (v1.60.0)
- [x] ch79 - A chainidx run --config flag (v1.61.0)
- [x] ch80 - Report network_magic (v1.62.0)
